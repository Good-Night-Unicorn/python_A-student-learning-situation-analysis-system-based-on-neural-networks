			...
			<div :style='{"width":"28%","padding":"0","flexDirection":"column","display":"flex","gap":"10px"}'>
				<div class="echarts1">
					...
					<div :style='{"width":"100%","padding":"0 0 0 120px","flex":"auto","height":"0"}'>
						<div id="learningdataChart1" style="width:100%;height:100%;"></div>
					</div>
				</div>
				<div class="echarts2">
					...
					<div :style='{"width":"100%","padding":"0 0 0 40px","flex":"auto","height":"0"}'>
						<div id="learningdataChart2" style="width:100%;height:100%;"></div>
					</div>
				</div>
			</div>
				...
				<div :style='{"padding":"0 10px","margin":"0","color":"#fff","alignItems":"center","display":"flex","justifyContent":"center","alignContent":"center","background":"none","gap":"10px","width":"100%","fontSize":"14px","height":"58px","order":"1"}'>
					...
					<div :style='{"padding":"0 10px","margin":"0","alignItems":"center","background":"url(http://codegen.caihongy.cn/20250104/9308fb76b74e4fa5be5d28c2f3ad95a4.png) no-repeat center top / 100% 100%","flexDirection":"column","flex":"1 1 auto","display":"flex","width":"150px","position":"relative","justifyContent":"center","height":"58px","maxWidth":"240px"}'>
						...
						<div :style='{"padding":"0","alignItems":"center","flexDirection":"row","display":"flex","gap":"6px","width":"100%","justifyContent":"space-evenly","height":"100%"}'>
							<div :style='{"color":"inherit","fontSize":"14px","height":"auto"}'>学习情况总数</div>
							<div :style='{"lineHeight":"44px","fontSize":"16px","color":"rgb(66, 144, 247)","fontWeight":"bold","height":"40px","order":"20"}'>{{learningdataCount}}</div>
						</div>
					</div>
				</div>
				<div class="echarts6">
					...
					<el-table id="learningdataTable" :data="learningdataList" border :stripe="false" :style='{"padding":"0","borderColor":"rgba(92,135,230,.3)","borderWidth":"1px 0 0 1px","background":"none","width":"100%","overflow-y":"scroll","borderStyle":"solid","height":"calc(100% - 36px)"}'>
						<el-table-column :resizable="true" :sortable="false" prop="student" label="学生">
						</el-table-column>
						<el-table-column :resizable="true" :sortable="false" prop="xingbie" label="性别">
						</el-table-column>
						<el-table-column :resizable="true" :sortable="false" prop="nianji" label="年级">
						</el-table-column>
						<el-table-column :resizable="true" :sortable="false" prop="banji" label="班级">
						</el-table-column>
						<el-table-column :resizable="true" :sortable="false" prop="subject" label="学科">
						</el-table-column>
						<el-table-column :resizable="true" :sortable="false" prop="regulargrade" label="平时成绩">
						</el-table-column>
						<el-table-column :resizable="true" :sortable="false" prop="midtermresults" label="期中成绩">
						</el-table-column>
						<el-table-column :resizable="true" :sortable="false" prop="finalgrade" label="期末成绩">
						</el-table-column>
						<el-table-column :resizable="true" :sortable="false" prop="zuoyewanchengqingkuang" label="作业完成情况">
						</el-table-column>
					</el-table>
				</div>
			</div>
			<div :style='{"width":"28%","padding":"0","flexWrap":"wrap","flexDirection":"column","display":"flex","gap":"10px"}'>
				<div class="echarts3">
					...
					<div :style='{"width":"100%","padding":"0 80px 0 0","flex":"auto","height":"0"}'>
						<div id="learningdataChart3" style="width:100%;height:100%;"></div>
					</div>
				</div>
				<div class="echarts5">
					<div :style='{"padding":"0 0 0 30px","margin":"0","textAlign":"left","background":"url(http://codegen.caihongy.cn/20250104/98a6bbd24bd24c75a26b8a0a8f28b0ca.png) no-repeat left center / 278px 36px","flex":"none","width":"100%","height":"36px"}'>
					...
					<div :style='{"width":"100%","padding":"0 120px 0 0","flex":"auto","height":"0"}'>
						<div id="learningdataChart5" style="width:100%;height:100%;"></div>
					</div>
				</div>
			</div>
		</div>
	...
	export default {
		...
		created() {
			this.$nextTick(()=>{
				this.times()
				setTimeout(()=>{
					this.getlearningdataCount();
					this.learningdataChat1();
					this.learningdataChat2();
					this.learningdataChat3();
					this.learningdataChat5();
					this.learningdataChat6();
				},500)
			})
		},
		methods:{
			getlearningdataList() {
				...
				this.$http({
					...
					if (data && data.code === 0) {
						this.learningdataList = data.data.list;
						let div = document.getElementById('learningdataTable')
						var scrollTop = -1
						function move() {
							if (div){
						...
						this.learningdataTimer = setInterval(move, 2000);
						div.addEventListener('mouseenter', e => {
							e.stopPropagation()
							clearInterval(this.learningdataTimer);
						})
			...
			getlearningdataCount() {
				this.$http({
					url: `learningdata/count`,
					method: "get"
				}).then(({
					data
				}) => {
					if (data && data.code == 0) {
						this.learningdataCount = data.data
					}
				})
			},
			//统计接口1
			learningdataChat1() {
				this.$nextTick(()=>{
					...
					var learningdataChart1 = echarts.init(document.getElementById("learningdataChart1"),'macarons');
					this.$http({
						...
						if (data && data.code === 0) {
							...
							// 使用刚指定的配置项和数据显示图表。
							learningdataChart1.setOption(option);
				

							//根据窗口的大小变动图表
							window.addEventListener('resize', () => {
								learningdataChart1.resize();
							},false);
						}else{
							this.$message({
								message: data.msg,
			...
			learningdataChat2() {
				this.$nextTick(()=>{
					...
					var learningdataChart2 = echarts.init(document.getElementById("learningdataChart2"),'macarons');
					this.$http({
						...
						if (data && data.code === 0) {
							...
							// 使用刚指定的配置项和数据显示图表。
							learningdataChart2.setOption(option);
				

							//根据窗口的大小变动图表
							window.addEventListener('resize', () => {
								learningdataChart2.resize();
							},false);
						}else{
							this.$message({
								message: data.msg,
			...
			learningdataChat3() {
				this.$nextTick(()=>{
					...
					var learningdataChart3 = echarts.init(document.getElementById("learningdataChart3"),'macarons');
					this.$http({
						...
						if (data && data.code === 0) {
							...
							// 使用刚指定的配置项和数据显示图表。
							learningdataChart3.setOption(option);
				

							//根据窗口的大小变动图表
							window.addEventListener('resize', () => {
								learningdataChart3.resize();
							},false);
						}else{
							this.$message({
								message: data.msg,
			...
			learningdataChat5() {
				this.$nextTick(()=>{
					...
					this.$http({
						url: "group/learningdata/xuexifangfa",
						method: "get",
					}).then(({ data }) => {
						if (data && data.code === 0) {
							let res = data.data;
							...
							var option = {};
							this.wordclouds(pArray,'learningdataChart5');
			...
			learningdataChat6() {
				this.$nextTick(()=>{
					...
					var learningdataChart6 = echarts.init(document.getElementById("learningdataChart6"),'macarons');
					this.$http({
						...
						if (data && data.code === 0) {
							...
							// 使用刚指定的配置项和数据显示图表。
							learningdataChart6.setOption(option);


							//根据窗口的大小变动图表
							window.addEventListener('resize', () => {
								learningdataChart6.resize();
							},false);
						}else{
							this.$message({
								message: data.msg,
