<template>
	<div class="home-content">
		...
		<div class="statis-box">
			<div id="statis1" class="statis1 animate__animated" v-if="isAuth('learningdata','首页总数')">
				<div class="left">
					<span class="icon iconfont icon-zhangjie8"></span>
				</div>
				<div class="right">
					<div class="num">{{learningdataCount}}</div>
					<div class="name">学习情况总数</div>
				</div>
			</div>
		</div>
		<!-- statis -->
		...
		<!-- 5 -->
		<div class="type5">
			<div id="learningdataChart1" class="echarts1 animate__animated" v-if="isAuth('learningdata','首页统计')"></div>
			<div id="learningdataChart2" class="echarts2 animate__animated" v-if="isAuth('learningdata','首页统计')"></div>
			<div id="learningdataChart3" class="echarts3 animate__animated" v-if="isAuth('learningdata','首页统计')"></div>
			<div id="learningdataChart4" class="echarts4 animate__animated" v-if="isAuth('learningdata','首页统计')"></div>
			<div id="learningdataChart5" class="echarts5 animate__animated" v-if="isAuth('learningdata','首页统计')"></div>
		</div>
	</div>
</template>
<script>
...
export default {
	...
	mounted(){
		...
		if(this.isAuth('learningdata','首页统计')){
	...
	methods:{
		handleScroll() {
			let arr = [
				...
				{id:'learningdataChart1',css:'animate__'},
				{id:'learningdataChart2',css:'animate__'},
				{id:'learningdataChart3',css:'animate__'},
				{id:'learningdataChart4',css:'animate__'},
				{id:'learningdataChart5',css:'animate__'},
			]
...
// 1234 折多
		...
		learningdataChat4() {
			this.$nextTick(()=>{
				...
				var learningdataChart4 = echarts.init(document.getElementById("learningdataChart4"),'macarons');
				this.$http({
					...
					if (data && data.code === 0) {
						...
						// 使用刚指定的配置项和数据显示图表。
						learningdataChart4.setOption(option);

						//根据窗口的大小变动图表
						window.onresize = function() {
							learningdataChart4.resize();
						};
					}else{
						this.$message({
							message: data.msg,
