...
def upload_csv_mapreduce_hadoop():
    # 查询数据
    query = "SELECT * FROM learningdata"
    df = pd.read_sql(query, connection)
    local_csv_path = os.path.join(parent_directory,"learningdata.csv")
    # 导出为 CSV 文件
    df.to_csv(local_csv_path, index=False)
    print(f"数据成功导出到 CSV 文件: {local_csv_path}")
    hdfs_csv_path = '/input/learningdata.csv'
    # 目标 HDFS 路径
    ...
    query = "SELECT * FROM xueshengchengji"
    df = pd.read_sql(query, connection)
    local_csv_path = os.path.join(parent_directory,"xueshengchengji.csv")
    # 导出为 CSV 文件
    df.to_csv(local_csv_path, index=False)
    print(f"数据成功导出到 CSV 文件: {local_csv_path}")
    hdfs_csv_path = '/input/xueshengchengji.csv'
    # 目标 HDFS 路径
    if hadoop_client.status(hdfs_csv_path,strict=False):
        # 删除HDFS上的文件
        hadoop_client.delete(hdfs_csv_path)
...
def send_cmd():
    ...
    [
        ...
        "-mapper", f"\"python group_mapper.py {csv_index('learningdata.csv','chuqinqingkuang')}\"",
        "-reducer", "\"python group_reducer.py chuqinqingkuang\"",
        "-input", "hdfs://localhost:9000/input/learningdata.csv",
        "-output", "hdfs://localhost:9000/output/learningdata/groupchuqinqingkuang"
    ],
    [
        f"{hadoop_path}/bin/hadoop.cmd", "jar", f"{hadoop_path}/share/hadoop/tools/lib/hadoop-streaming-3.3.0.jar",
        "-files", "\"hdfs://localhost:9000/input/group_mapper.py,hdfs://localhost:9000/input/group_reducer.py\"",
        "-mapper", f"\"python group_mapper.py {csv_index('learningdata.csv','learningattitude')}\"",
        "-reducer", "\"python group_reducer.py learningattitude\"",
        "-input", "hdfs://localhost:9000/input/learningdata.csv",
        "-output", "hdfs://localhost:9000/output/learningdata/grouplearningattitude"
    ],
    [
        f"{hadoop_path}/bin/hadoop.cmd", "jar", f"{hadoop_path}/share/hadoop/tools/lib/hadoop-streaming-3.3.0.jar",
        "-files", "\"hdfs://localhost:9000/input/group_mapper.py,hdfs://localhost:9000/input/group_reducer.py\"",
        "-mapper", f"\"python group_mapper.py {csv_index('learningdata.csv','ketangbiaoxian')}\"",
        "-reducer", "\"python group_reducer.py ketangbiaoxian\"",
        "-input", "hdfs://localhost:9000/input/learningdata.csv",
        "-output", "hdfs://localhost:9000/output/learningdata/groupketangbiaoxian"
    ],
    [
        f"{hadoop_path}/bin/hadoop.cmd", "jar", f"{hadoop_path}/share/hadoop/tools/lib/hadoop-streaming-3.3.0.jar",
        "-files", "\"hdfs://localhost:9000/input/group_mapper.py,hdfs://localhost:9000/input/group_reducer.py\"",
        "-mapper", f"\"python group_mapper.py {csv_index('learningdata.csv','xuexihuanjing')}\"",
        "-reducer", "\"python group_reducer.py xuexihuanjing\"",
        "-input", "hdfs://localhost:9000/input/learningdata.csv",
        "-output", "hdfs://localhost:9000/output/learningdata/groupxuexihuanjing"
    ],
    [
        f"{hadoop_path}/bin/hadoop.cmd", "jar", f"{hadoop_path}/share/hadoop/tools/lib/hadoop-streaming-3.3.0.jar",
        "-files", "\"hdfs://localhost:9000/input/group_mapper.py,hdfs://localhost:9000/input/group_reducer.py\"",
        "-mapper", f"\"python group_mapper.py {csv_index('learningdata.csv','xuexifangfa')}\"",
        "-reducer", "\"python group_reducer.py xuexifangfa\"",
        "-input", "hdfs://localhost:9000/input/learningdata.csv",
        "-output", "hdfs://localhost:9000/output/learningdata/groupxuexifangfa"
    ],
    [
        f"{hadoop_path}/bin/hadoop.cmd", "jar", f"{hadoop_path}/share/hadoop/tools/lib/hadoop-streaming-3.3.0.jar",
        "-files", "\"hdfs://localhost:9000/input/group_mapper.py,hdfs://localhost:9000/input/group_reducer.py\"",
        "-mapper", f"\"python group_mapper.py {csv_index('learningdata.csv','xingquaihao')}\"",
        "-reducer", "\"python group_reducer.py xingquaihao\"",
        "-input", "hdfs://localhost:9000/input/learningdata.csv",
        "-output", "hdfs://localhost:9000/output/learningdata/groupxingquaihao"
    ],
    ]
